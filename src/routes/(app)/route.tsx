...
export const Route = createFileRoute('/(app)')({
  beforeLoad: async ({ context }) => {
    const activeSession = sessionExists(context.supabase)
    if (!activeSession) throw redirect({ to: '/login' })
    const user_id = getUserId(context.supabase)
    if (!user_id) throw redirect({ to: '/login' })
    const profile_id = getUserId(context.supabase)
    if (!profile_id)
      throw new Error(
        'Your account was set up incorrectly. User has no linked profile.',
      )
    const permissions = getPermissions(context.supabase)
