...
export const holiday_dates = createCollection(
  queryCollectionOptions({
    ...
    queryFn: async () => {
      ...
      const parsed = data.map((item) => ZodHolidayDatesRow.parse(item))
      return parsed
    },
    queryClient,
    schema: ZodHolidayDatesRow,
    getKey: (item) => item.id,
    onInsert: async ({ transaction, collection }) => {
      const localNewItems = transaction.mutations.map((m) => m.modified)

      const parsedLocalNewItems = localNewItems.map((item) =>
        ZodHolidayDatesInsertToDb.parse(item),
      )
    ...
    onUpdate: async ({ transaction, collection }) => {
      ...
      const parsedChangesToApply =
        ZodHolidayDatesUpdateToDb.parse(localChangesToApply)
