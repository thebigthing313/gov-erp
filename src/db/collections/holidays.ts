...
export const holidays = createCollection(
  queryCollectionOptions({
    ...
    queryFn: async () => {
      ...
      const parsed = data.map((item) => ZodHolidaysRow.parse(item))
      return parsed
    },
    queryClient,
    ...
    onInsert: async ({ transaction, collection }) => {
      ...
      const parsedLocalNewItems = localNewItems.map((item) =>
        ZodHolidaysInsertToDb.parse(item),
      )
    ...
    onUpdate: async ({ transaction, collection }) => {
      ...
      const parsedChangesToApply =
        ZodHolidaysUpdateToDb.parse(localChangesToApply)
