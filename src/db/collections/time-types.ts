...
export const time_types = createCollection(
  queryCollectionOptions({
    ...
    queryFn: async () => {
      ...
      const parsed = data.map((item) => ZodTimeTypesRow.parse(item))
      return parsed
    },
    queryClient,
    schema: ZodTimeTypesRow,
    staleTime: Infinity,
    getKey: (item) => item.id,
    onInsert: async ({ transaction, collection }) => {
      ...
      const parsedLocalNewItems = localNewItems.map((item) =>
        ZodTimeTypesInsertToDb.parse(item),
      )
    ...
    onUpdate: async ({ transaction, collection }) => {
      ...
      const parsedChangesToApply =
        ZodTimeTypesUpdateToDb.parse(localChangesToApply)
